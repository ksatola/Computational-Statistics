...
Solving the problem analytically, we would have a problem with the more complex scenario (7 dice, sum greater than 35) whereas computationally we can solve it right away.

The working code and complex case solution can be found the Jupyter notebook format below.
...
We represent our groups as lists where each number is a result of patient's examination. In our case we do not know what the numbers means and if the higher number the better result is. That is why two-tailed alternative hypothesis is chosen - we just want to confirm if the two gorups differ significanly.

 The treatment group is where patients get the drug and the control group is where patients do not get drug.
...
For our groups, we observe the means difference of 12.97 but the question is if this difference is statistically significant and the drug works? Is the difference only between the two samples (drug and placebo) or we could infere that this would be also valid for entire patients population?
...
First, let's try to answer to this questions analytically. For that we will use T-Student sampling distribution and calculate t-test statistic. Statistical tests usually come with some assumptions, and t-test is not an exception.
...
Assumtions 3 and 4 are quick to deal with. Our first sample (drug) has 9 observations and the second (placebo) 10. They are not equal in size but close, so we can live with it. The small number of observations (less than 20) may impact the quality of t-test outcome but we have no other choice but try (we do not have any other observations).

To check if assumption one is our reality we will use Shapiro-Wilk test designed specifically for samples containing less than 50 observations. For that we will use a helper function `normaltest()` which utilizes `shapiro()` function of the `scipy.stats` module.
...
Our samples come from a normal distribution, although it is difficult to say that looking at them because we have only 9 and 10 observations.
...
To check if the assumption is valid, we will use `varsequals()` helper function utilizing `bartlett()` function from `scipy.stats` module. The function uses Bartlett's test if our samples come from populations with equal variances.
...
```
I seems that our second assumption is also valid.
...
If there is no difference in means of the treatment and control groups, we can merge the observations from both groups and then check how likely is that the observed_diff value or greater appears. We will now simulate the mean differences doing random groups assignment from the common pool many times.
...
for _ in range(n):
    ...
    shuffled_diff = mean(combined[:len(drug)]) 
...
```
If the null hypothesis is true (the drug does not work, on average group means are close to 0) we should observe many occurences of mean differences equal or greater to observed_diff. If not, then we can safely reject the null hypothesis.

Our simulation result shows that 12 times on 10,000 simulated mean difference was equal or greater then observed one. According to our logic, if such difference was common we would get the difference at list 12.97 many more times.
...
```
We can also calculate p-value easily. To recall, p-value is a chance of observing the current difference when there is truly no difference. In other words it is a probability of obtaining a result equal to or more extreme than was observed in the data.
...
The picture above shows our simulated distribution of mean difference. The green vertical line represent the critical value and the red one the mean difference observed between drug and placebo groups. We clearly see that it is very unlikely that drug and placebo samples come from the same distribution.
...
The analytical and computational approach are similar in most places but the core part differs (in bold below):
...
To conclude, analytical method often dictates the test statistic, and tests have assumptions. This is not the case while simulating. In addition, if you have issues like censored data, non-independence, or long-tailed distributions, you may not find an off-the-shelf test. In such case, unless you are a mathematical statistician, you may not be able to make it by yourself. Computational approach seems to be more easy to comprehend and to execute.
